...
function [MI_matrix] = calc_MI_ozkurt_test(virtsens,phase,amp,diag)
...
for k = phase(1):1:phase(2)
    for p = amp(1):2:amp(2)
        ...
        for trialnum = 1:length(virtsens.trial)
            
            [pha, amp] = pa_series(virtsens.trial{1,trialnum},virtsens.trial{1,trialnum},[Pf1,Pf2],[Af1,Af2],1000);
            
            if (length(pha)/2)+1500 > length(pha)
                pha_post = pha((length(pha)/2)+300:end);
                amp_post = amp((length(amp)/2)+300:end);
                
            else
                pha_post = pha((length(pha)/2)+300:((length(pha)/2)+1500));
                amp_post = amp((length(amp)/2)+300:((length(amp)/2)+1500));
            end
            
            [m_raw1, m_raw2] = modulation_index_ozkurt_nostats(amp_post, pha_post);
            MI_comb(trialnum) = m_raw2; % m_raw2 is the Özkurt method
        end
        ...
        % Add to Matrix
        MI_matrix(row1,row2) = mean(MI_comb);
